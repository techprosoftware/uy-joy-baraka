    ...
    debounceMe(async (evt) => {
      ...
      if (currentValue !== "" && currentValue.trim()) {
        try {
          ...
          //* Stores the incoming posts to unique posts as JSON
          response.data.posts.forEach((post) =>
            uniquePosts.add(JSON.stringify(post))
          );

          //* Stores the incoming translated posts to unique posts as JSON
          responseLatin.data.posts.forEach((post) =>
            uniquePosts.add(JSON.stringify(post))
          );

          //* Collect as a combined posts(texts)
          //* Yangi object
          const combinedPosts = Array.from(uniquePosts).map((postString) =>
              ...
              >
                ...
                <li className="dropdown-item">
                  <button
                    ...
                    className="send__button w-100 mt-2"
                    // href="#"
                  >
                    {t("searchpage.filter")}
                  </button>
